...
class Decide:
    ...
    def get_all_pips(self):
        ...
        if decision_buy > decision_sell and win_loss_buy_ratio >= 1.0: 
            ...
            self.direction = 1
            self.take_profit = round(buy_decision_tp['Take Profit'], 3)
            ...
            self.direction = -1
            self.take_profit = round(sell_decision_tp['Take Profit'], 3)
            self.stop_loss = round(sell_decision_sl['Stop Loss'], 3)   
        else:
            self.decision = f'\nNeutral \nBuy utility:{round(decision_buy,2)})'
            self.decision += f"\nBuy Gain: ${sell_decision_tp['portfolio_gain'] - portfolio}"
            self.decision += f"\nBuy Loss: ${abs(portfolio - buy_decision_sl['portfolio_loss'])}"
            self.decision += f'\nWin/Loss ratio Buy: {win_loss_buy_ratio}'
            self.decision += f"\nSell utility:{round(decision_sell,2)})"
