...
class Data():
    def __init__(self, **setup):
        ...
        Input:
            - **setup(dict):
                ...
                - trading: (boolean.optional) True if it is used for 
                live trading
        '''
    ...
    def get_data(self, model_version):
        ...
        try:
            assert Xh.shape[-1] == len(scaler_high.mean_)
            assert Xl.shape[-1] == len(scaler_low.mean_)
        except AssertionError as e:
            logging.error(f'{e} número de variables distinto al número de '
                          'variables de escaladores')
            logging.error(
                f'(High)Data shape: {Xh.shape[-1]} scaler shape: {len(scaler_high.mean_)}')
            logging.error(
                f'(Low)Data shape: {Xl.shape[-1]} scaler shape: {len(scaler_low.mean_)}')
